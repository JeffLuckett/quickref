...
function bte_wb_options() {	 	
	...
	if (!empty($_POST['bte_wb_action'])) {
		...
		if (isset($_POST['bte_wb_deletefile'])) {
			update_option('bte_wb_deletefile',$_POST['bte_wb_deletefile']);
		}
	...
	$bte_wb_deletefile = get_option('bte_wb_deletefile');		
	if(empty($bte_wb_deletefile)) {
		$bte_wb_deletefile = true;
		update_option('bte_wb_deletefile',$bte_wb_deletefile);
	}
	...
	print('
				...
				<h3>Wordpress Backup Options</h3><form id="bte_wb" name="bte_wordpressbackup" action="'.get_bloginfo('wpurl').'/wp-admin/options-general.php?page=BTE_WB_admin.php" method="post">
					...
					<fieldset class="options">
						...
						<!--
						<div class="option">
							<label for="bte_wb_deletefile">'.__('Delete File After Email: ', 'WordpressBackup').'</label>
							<select name="bte_wb_deletefile" id="bte_wb_deletefile">
									<option value="1" '.bte_wb_optionselected(1,$bte_wb_deletefile).'>'.__('Yes', 'WordpressBackup').'</option>
									<option value="0" '.bte_wb_optionselected(0,$bte_wb_deletefile).'>'.__('No', 'WordpressBackup').'</option>
							</select>');
							print('<font color="red">'.__('If you leave the files on the server please add an .htaccess file to the backup folder.', 'WordpressBackup').'</font>
						</div> -->
						<div class="option">
							...
							if (!file_exists($bte_wb_backup_dir.'/.htaccess')) {
								print('<font color="red">'.__('Please add an .htaccess file to the backup folder (see <a href="http://www.blogtrafficexchange.com/wordpress-backup/">here</a>).', 'WordpressBackup').'</font>');
							}
							...
							if(@file_exists(stripslashes($bte_wb_backup_dir.'/themes.zip')) && @is_writable(stripslashes($bte_wb_backup_dir.'/themes.zip'))) {				
								print('<p><font color="green">'.__('themes.zip is writeable', 'WordpressBackup').'</font></p>');
							} else if(@file_exists(stripslashes($bte_wb_backup_dir.'/themes.zip'))) {
								print('<font color="red">'.__('themes.zip is NOT writable. Please CHMOD it to \'666\'.', 'WordpressBackup').'</font>');								
							}
	print('												
						</div>
						<div class="option">
