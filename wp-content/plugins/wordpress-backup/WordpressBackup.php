...
if (!defined('BTE_WB_DIR')) {
	//define('BTE_WB_DIR', WP_CONTENT_DIR.'/bte-wb'.rand());
...
add_filter('plugin_action_links', 'bte_wb_plugin_action_links', 10, 2);

function bte_wb_plugin_action_links($links, $file) {
	$plugin_file = basename(__FILE__);
	if (basename($file) == $plugin_file) {
		$settings_link = '<a href="options-general.php?page=BTE_WB_admin.php">'.__('Settings', 'WordpressBackup').'</a>';
...
function bte_wb_activate() {
	add_option('bte_wb_deletefile',true);
...
function bte_wb_wordpress_backup () {
	...
	if (@is_dir(stripslashes($bte_wb_dir)) && @is_writable(stripslashes($bte_wb_dir))) {
		if (@is_dir(stripslashes(get_option("bte_wb_upload_dir"))) && (bte_wb_is_time('bte_wb_last_backup_uploads'))) {
			$now = time();
			bte_wb_backup(get_option("bte_wb_upload_dir"),"uploads",date('Y-m-d H:i:s',$now),'bte_wb_last_backup_uploads',$now);
		} else if (@is_dir(stripslashes(get_option("bte_wb_template_dir"))) && (bte_wb_is_time('bte_wb_last_backup_theme'))) {
			$now = time();
			bte_wb_backup(get_option("bte_wb_template_dir"),"themes",date('Y-m-d H:i:s',$now),'bte_wb_last_backup_theme',$now);
		} else if (@is_dir(stripslashes(get_option("bte_wb_plugin_dir"))) && (bte_wb_is_time('bte_wb_last_backup_plugin'))) {
			$now = time();
	...
	if( !ini_get('safe_mode') ) {
  	...
  	if ($archive->create($dir,PCLZIP_OPT_REMOVE_PATH, $v_remove, PCLZIP_OPT_ADD_PATH, $loc) == 0) {
		...
		//chmod("$bte_wb_dir/$loc.zip",666);
  		...
  		if (get_option("bte_wb_deletefile")) {
  			//unlink("$bte_wb_dir/$loc.zip");
  		}
  	}
}
